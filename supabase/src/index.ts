...
function safeError(e: any, res: express.Response, statusCode = 500): void {
...
app.get('/api/members', requireAuth, async (req, res) => {
    try {
        ...
        const userRole = (req as any).memberRole;
...
app.put('/api/members/:id', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const orgId = (req as any).organizationId;
        const requesterRole = (req as any).memberRole;
        const authUser = (req as any).authUser;
        ...
        if (isAdmin) {
            ...
            if (pay_rate !== undefined) updatePayload.pay_rate = pay_rate;
            if (bill_rate !== undefined) updatePayload.bill_rate = bill_rate;
            if (weekly_limit !== undefined) updatePayload.weekly_limit = weekly_limit;
            if (daily_limit !== undefined) updatePayload.daily_limit = daily_limit;
            if (tracking_enabled !== undefined) updatePayload.tracking_enabled = tracking_enabled;
        } else {
...
app.post('/api/sessions/:id/end', requireAuth, async (req, res) => {
    try {
        ...
        const sessionOwnerAuthId = (session.members as any)?.auth_user_id;
...
app.get('/api/payments', requireAuth, async (req, res) => {
    try {
        ...
        const filteredData = (data || []).filter((p: any) => p.members?.organization_id === orgId);
