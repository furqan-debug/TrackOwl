  ...
  try {
    ...
    if (isScheduled) {
      ...
      try {
        ...
        // The amount due today is the amount_due on this upcoming invoice that's generated by the proration
        ...
        // If Monthly -> Yearly switch, Stripe immediately resets the billing cycle,
        // so the 'upcoming' invoice period_end will be 1 year from now.
        if (newBillingCycle === "Yearly" && org.subscription_period === "Monthly") {
            nextRenewalDate = new Date(upcoming.period_end * 1000).toISOString();
        }

      } catch (err: any) {
         console.warn("Failed to fetch upcoming invoice, using fallback math:", err.message);
         // Fallback if Stripe API fails (e.g. testing with invalid keys)
         amountDueToday = 0; 
      }
    }
