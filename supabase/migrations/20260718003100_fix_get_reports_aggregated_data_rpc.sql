-- ==============================================================
-- DigiReps Tracker — Fix: Fully Aggregated Reports RPC
-- Bug: productive CTE was only scoped to first SELECT INTO,
--      subsequent queries failed with "relation productive does not exist"
-- Fix: Materialize productive rows into a temp table first,
--      then run the three aggregations independently.
-- ==============================================================
...
DECLARE
  ...
  v_apps jsonb;
BEGIN
  -- Materialize productive minutes into a temp table so we can
  -- query it three times without repeating the expensive CTE chain.
  CREATE TEMP TABLE _productive ON COMMIT DROP AS
  WITH deduped AS (
  ...
  WHERE (b.manual = true)
     OR (b.idle = false OR COALESCE(m.idle_limit, 0) <= 1 OR bc.block_len <= m.idle_limit);
