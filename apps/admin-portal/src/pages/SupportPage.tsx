...
function Layout({ children, search, setSearch }: { children: React.ReactNode; search: string; setSearch: (s: string) => void }) {
  ...
  const searchResults = useMemo(() => {
    ...
    return articles
      .map(art => {
        let score = 0;
        if (art.title.toLowerCase() === query) score += 100;
        else if (art.title.toLowerCase().startsWith(query)) score += 50;
        else if (art.title.toLowerCase().includes(query)) score += 20;
        else if (art.category.toLowerCase().includes(query)) score += 10;
        else if (art.content.toLowerCase().includes(query)) score += 1;
        return { art, score };
      })
    ...
    return (
        <div className="support-form-container">
            <div className="support-form-header">
                <div>
                    <h3 className="support-form-title" style={{ color: '#ffffff' }}>Contact TrackOwl</h3>
...
function HomePageView({ search, setSearch }: { search: string; setSearch: (s: string) => void }) {
  const navigate = useNavigate();
        ...
        <div className="home-hero-banner-content animate-fade-in">
          ...
          <div className="hero-search-wrapper">
            ...
            <input 
              ...
              placeholder="Search our knowledge base, categories, guides..." 
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            {search && searchResults.length > 0 && (
              <div className="hero-search-results">
                {searchResults.map(res => (
                  ...
                  >
                    <div className="hero-search-result-title">{res.title}</div>
                    <div className="hero-search-result-category">{res.category}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="contact-section-wrapper">
          <div className="contact-section-grid">
            <div className="contact-info-col">
              <h2 className="contact-info-title">Need direct assistance?</h2>
              <p className="contact-info-desc">
                Can't find what you are looking for in our documentation? Contact our dedicated support team, schedule a live demo, or talk directly to our sales department.
              </p>
              ...
              <div className="contact-info-cards">
                <div className="contact-info-card">
                  ...
                  <div>
                    <h4>Email Support</h4>
                    <p>Send us an email at <a href="mailto:contact@trackowl.io">contact@trackowl.io</a></p>
                  </div>
                </div>
                ...
                <div className="contact-info-card">
                  ...
                  <div>
                    <h4>Response Time</h4>
                    <p>Average response time is under 2 hours during business days.</p>
                  </div>
                </div>
              </div>
            </div>
  ...
  if (!categoryArticles.length) {
    return (
      <div className="not-found">
        ...
        <Link to="/support" className="btn-primary">Return to Home</Link>
      </div>
    );
  }
...
export function SupportPage() {
  ...
  return (
    <Layout search={search} setSearch={setSearch}>
      <Routes>
        <Route path="/" element={<HomePageView search={search} setSearch={setSearch} />} />
        <Route path="/category/:cat" element={<CategoryPageView />} />
        <Route path="/article/:slug" element={<ArticlePageView />} />
      </Routes>
    </Layout>
  );
}
