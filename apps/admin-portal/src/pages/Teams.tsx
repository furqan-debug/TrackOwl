            ...
            <Modal
                ...
                subtitle={(
                    <div className="flex items-center gap-3 mt-2">
                        ...
                        <span className="text-[11px] font-bold text-text-muted uppercase tracking-wider">Step {wizardStep} of 3</span>
                    </div>
                )}
                footer={
                            ...
                            <div className="flex items-center gap-4">
                                {wizardStep < 3 ? (
                                    <Button
                                        ...
                                        className="px-8 shadow-shell-sm"
                                    >
                                        Next
                                    </Button>
                                ) : (
                        ...
                        <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                            ...
                            <div className="space-y-2 relative" ref={dropdownRef}>
                                ...
                                <div 
                                    ...
                                    className={clsx(
                                        "w-full flex items-center justify-between bg-surface-solid border rounded-xl px-4 py-3 cursor-pointer transition-all shadow-shell-sm",
                                        managerDropdownOpen ? "border-primary ring-2 ring-primary/20" : "border-border hover:border-text-muted/30"
                                    )}
                                >
                                    {managerId ? (
                                        <div className="flex items-center gap-3">
                                            <div className="w-7 h-7 rounded-lg bg-primary text-white flex items-center justify-center text-[11px] font-bold shadow-shell-sm">
                                                {members.find(m => m.id === managerId)?.full_name[0].toUpperCase()}
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="text-[13px] font-bold text-text-primary leading-none mb-0.5">{members.find(m => m.id === managerId)?.full_name}</span>
                                                <span className="text-[10px] font-bold text-text-muted">{members.find(m => m.id === managerId)?.email}</span>
                                            </div>
                                        </div>
                                    ) : (
                                        <span className="text-[13px] font-medium text-text-muted">Select a manager...</span>
                                    )}
                                    <ChevronDown className={clsx("w-4 h-4 text-text-muted transition-transform duration-300", managerDropdownOpen && "rotate-180")} />
                                </div>
                                ...
                                {managerDropdownOpen && (
                                    <div className="absolute top-[100%] left-0 w-full mt-2 bg-surface border border-border rounded-2xl shadow-2xl z-[100] max-h-[260px] overflow-y-auto custom-scrollbar p-2 animate-in fade-in zoom-in-95 duration-200">
                                        {members.map(m => (
                                            ...
                                            >
                                                <div className={clsx(
                                                    "w-9 h-9 rounded-[10px] flex items-center justify-center text-[12px] font-bold transition-all shadow-sm",
